...
def map_canonical_error(error: CanonicalError) -> AnthropicProtocolError:
    ...
    if error.category is ErrorCategory.OVERLOADED:
        return AnthropicProtocolError(529, "overloaded_error", error.message)
    if error.category in {
        ErrorCategory.INVALID_REQUEST,
        ErrorCategory.UNSUPPORTED_CAPABILITY,
        ErrorCategory.CONTEXT_LENGTH,
    }:
        return AnthropicProtocolError(400, "invalid_request_error", error.message)
    return AnthropicProtocolError(500, "api_error", error.message)
