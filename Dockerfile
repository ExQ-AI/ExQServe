...
RUN python -m pip install --no-cache-dir . \
    && python -c "import torch; import exllamav3_ext, fastapi, huggingface_hub, jsonschema, uvicorn; import exllamav3, exqserve; from importlib.metadata import version; assert version('exllamav3').startswith('1.4.4+cu128.torch2.11.0'); assert version('exqserve') == '0.1.0'; assert torch.__version__.startswith('2.11.0')"
